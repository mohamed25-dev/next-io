      ...
      <ListItemText
        primary={
          <Link href={`/question/${item.id}`} passHref>
            <Typography className={classes.link}>{item.question.title}</Typography>
          </Link>
        }
        secondary={
          <Box display="flex">
...
function Tags({ tags = [] }) {
  return tags.map(tag =>
    <Link href={`/tag/${tag.slug}`} passHref key={tag.id}>
      <Chip label={tag.name} color="secondary" variant="default" size="small" />
    </Link>
  )
}
