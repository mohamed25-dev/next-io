...
import { Content, Tags, } from 'components/question';
...
export default function Show({ params }) {
  ...
  return (
    <MainLayout title={post?.question?.title} loading={loading}>
      ...
      <Answers vote={vote} items={post?.answers}/>
      {
...
function Answer({ data: { id, content, user, createdAt, votesTotal }, vote }) {
  return (
    <Box p={2}>
      <Box display="flex">
        <Vote votesTotal={votesTotal} vote={type => vote(id, type)} />
